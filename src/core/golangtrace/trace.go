...
It exports HTTP interfaces on /debug/pprof/requests and /debug/pprof/events.
...
The /debug/pprof/requests HTTP endpoint organizes the traces by family,
...
The /debug/pprof/events HTTP endpoint organizes the event logs by family and
...
import (
	...
	"github.com/zxfonline/IMDemo/core/golangtrace/timeseries"
)
...
// HTTP ServeMux paths.
const (
	debugRequestsPath = "/debug/pprof/requests"
	debugEventsPath   = "/debug/pprof/events"
)

// AuthRequest determines whether a specific request is permitted to load the
// /debug/pprof/requests or /debug/pprof/events pages.
//
...
func init() {
	...
	if pat == debugRequestsPath {
		panic("/debug/pprof/requests is already registered. You may have two independent copies of " +
...
// Render renders the HTML page typically served at /debug/pprof/requests.
...
//获取所有可用于/debug/pprof/vars 调用的请求信息
...
type Trace interface {
	...
	// /debug/pprof/requests page is rendered. Any memory referenced by x will be
	// pinned until the trace is finished and later discarded.
	LazyLog(x fmt.Stringer, sensitive bool)

	// LazyPrintf evaluates its arguments with fmt.Sprintf each time the
	// /debug/pprof/requests page is rendered. Any memory referenced by a will be
	// pinned until the trace is finished and later discarded.
...
type traceSet struct {
	...
	// However, that would shift some of the expense from /debug/pprof/requests time to RPC time,
...
<html>
	<head>
	<title>/debug/pprof/requests</title>
	<style type="text/css">
		body {
